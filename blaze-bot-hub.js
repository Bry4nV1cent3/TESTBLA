...
// @version      7.0
// @description  Hub de automação para Blaze Mines e Double. Detecta o jogo e carrega a interface e lógica corretas.
// @author       Você
...
(function() {
    ...
    // --------------------------------------------------------------------------------------
    //  LÓGICA DO BOT PARA O JOGO MINES
    // --------------------------------------------------------------------------------------
    function initMinesBot() {
        // [CÓDIGO DO BOT DE MINES - INÍCIO]
        const CONFIG = {
            apostaAutomaticaAtiva: false,
            valorBaseAposta: 1,
            numeroDeMinas: 3,
            clicksParaRetirar: 5,
            estrategiaDeCliques: [6, 7, 8, 11, 12, 13, 16, 17, 18],
        };
        ...
        const STATE = {
            ...
            clicksFeitos: 0,
            estatisticas: { vitorias: 0, derrotas: 0 },
        };

        const Acao = {
            iniciarJogo() {
                const betButton = document.querySelector('[data-testid="bet-button"]');
                if (!betButton) { UI.updateStatus("❌ Botão de aposta não encontrado.", 'error'); return; }
                betButton.click();
                UI.updateStatus("🚀 Jogo iniciado...", 'info');
                STATE.jogoEmAndamento = true;
                STATE.clicksFeitos = 0;
                setTimeout(Acao.clicarProximo, 2000);
            },
            clicarProximo() {
                if (!STATE.jogoEmAndamento) return;
                if (STATE.clicksFeitos >= CONFIG.clicksParaRetirar) return Acao.retirarPremio();
                const mines = document.querySelectorAll('.cell.closed');
                if (mines.length === 0) {
                    UI.updateStatus("⚠️ Nenhuma célula para clicar. Aguardando novo jogo...", 'warning');
                    STATE.jogoEmAndamento = false;
                    setTimeout(Acao.verificarEIniciar, 3000); return;
                }
                const indice = CONFIG.estrategiaDeCliques[STATE.clicksFeitos];
                const mine = mines[indice];
                if (mine) {
                    mine.click();
                    STATE.clicksFeitos++;
                    UI.updateStatus(`➡️ Clique ${STATE.clicksFeitos}/${CONFIG.clicksParaRetirar} em andamento...`, 'info');
                    setTimeout(Acao.clicarProximo, 1000);
                } else {
                    UI.updateStatus("⚠️ Estratégia de cliques esgotada ou célula já aberta.", 'warning');
                    Acao.retirarPremio();
                }
            },
            retirarPremio() {
                const cashoutButton = document.querySelector('.mines-cashout-button');
                if (!cashoutButton) { UI.updateStatus("❌ Botão de 'Retirar' não encontrado. Jogo encerrado.", 'error'); STATE.jogoEmAndamento = false; STATE.estatisticas.derrotas++; UI.updateStats(); return setTimeout(Acao.verificarEIniciar, 3000); }
                cashoutButton.click();
                UI.updateStatus("🏆 Prêmio retirado com sucesso!", 'win');
                STATE.jogoEmAndamento = false;
                STATE.estatisticas.vitorias++;
                UI.updateStats();
                setTimeout(Acao.verificarEIniciar, 3000);
            },
            ajustarConfiguracoes() {
                const minesInput = document.querySelector('[data-testid="mines-input"]');
                const betInput = document.querySelector('[data-testid="bet-input"]');
                if (minesInput) { minesInput.value = CONFIG.numeroDeMinas; minesInput.dispatchEvent(new Event('input', { bubbles: true })); }
                if (betInput) { betInput.value = CONFIG.valorBaseAposta; betInput.dispatchEvent(new Event('input', { bubbles: true })); }
            },
            verificarEIniciar() {
                if (!CONFIG.apostaAutomaticaAtiva || STATE.jogoEmAndamento) return;
                const betButton = document.querySelector('[data-testid="bet-button"]');
                if (betButton && betButton.textContent.trim() === 'Começar o jogo') {
                    Acao.ajustarConfiguracoes();
                    setTimeout(Acao.iniciarJogo, 1000);
                } else {
                    setTimeout(Acao.verificarEIniciar, 1000);
                }
            }
        };
        ...
        const UI = {
            ...
            init() {
                ...
                this.painel.style.cssText = `position: fixed; top: 20px; right: 20px; background: rgba(20, 20, 20, 0.9); color: white; padding: 15px; border-radius: 12px; width: 300px; z-index: 99999; font-family: 'Segoe UI', sans-serif; backdrop-filter: blur(5px); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3); border: 1px solid rgba(255, 255, 255, 0.1); cursor: move; user-select: none; transition: transform 0.3s ease-out;`;
                this.painel.innerHTML = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;"><h3 style="margin: 0; font-size: 16px; color: #FFD700;">⛏️ BLAZE BOT MINES</h3><div id="stats" style="font-size: 12px;">✅ 0 | ❌ 0</div></div><div id="status" style="padding: 8px; border-radius: 6px; background: rgba(0, 0, 0, 0.3); margin-bottom: 12px; font-size: 13px; min-height: 18px;">Status: Pronto para jogar.</div><div id="configs"><h4 style="margin: 0 0 8px 0; font-size: 14px; color: #FFD700;">Configurações</h4><div id="aposta-container" style="display: flex; flex-direction: column; gap: 8px;"><label style="display: flex; align-items: center; gap: 5px; font-size: 12px;"><input type="checkbox" id="toggleApostaAuto">Ativar Automação</label><div style="display: flex; justify-content: space-between; gap: 10px;"><div style="display: flex; flex-direction: column;"><label style="font-size: 12px;">Valor Base:</label><input type="number" id="inputValorAposta" value="${CONFIG.valorBaseAposta}" min="1" step="1" style="width: 80px; background: #333; border: 1px solid #555; color: white; padding: 5px;"></div><div style="display: flex; flex-direction: column;"><label style="font-size: 12px;">Nº de Minas:</label><input type="number" id="inputNumeroMinas" value="${CONFIG.numeroDeMinas}" min="2" max="24" step="1" style="width: 80px; background: #333; border: 1px solid #555; color: white; padding: 5px;"></div></div><div style="display: flex; flex-direction: column;"><label style="font-size: 12px;">Cliques p/ Retirar:</label><input type="number" id="inputCliquesRetirar" value="${CONFIG.clicksParaRetirar}" min="1" max="24" step="1" style="width: 100%; background: #333; border: 1px solid #555; color: white; padding: 5px;"></div></div></div>`;
                document.body.appendChild(this.painel);
                this.makeDraggable(); this.setupListeners();
            },
            makeDraggable() { /* ... */ }, setupListeners() { /* ... */ }, updateStats() { /* ... */ }, updateStatus(text, type) { /* ... */ }
        };

        UI.makeDraggable = function() {
            const panel = this.painel; let pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0; panel.onmousedown = dragMouseDown;
            function dragMouseDown(e) { e = e || window.event; e.preventDefault(); pos3 = e.clientX; pos4 = e.clientY; document.onmouseup = closeDragElement; document.onmousemove = elementDrag; }
            function elementDrag(e) { e = e || window.event; e.preventDefault(); pos1 = pos3 - e.clientX; pos2 = pos4 - e.clientY; pos3 = e.clientX; pos4 = e.clientY; panel.style.top = (panel.offsetTop - pos2) + "px"; panel.style.right = "unset"; panel.style.left = (panel.offsetLeft - pos1) + "px"; }
            function closeDragElement() { document.onmouseup = null; document.onmousemove = null; }
        };
        UI.setupListeners = function() {
            const toggleAposta = this.painel.querySelector('#toggleApostaAuto');
            const inputValor = this.painel.querySelector('#inputValorAposta'); const inputMinas = this.painel.querySelector('#inputNumeroMinas'); const inputCliques = this.painel.querySelector('#inputCliquesRetirar');
            toggleAposta.checked = CONFIG.apostaAutomaticaAtiva;
            toggleAposta.addEventListener('change', (e) => { CONFIG.apostaAutomaticaAtiva = e.target.checked; this.updateStatus(`Automação ${e.target.checked ? 'ativada' : 'desativada'}.`, 'info'); if (e.target.checked) Acao.verificarEIniciar(); });
            inputValor.addEventListener('change', (e) => { CONFIG.valorBaseAposta = parseFloat(e.target.value); this.updateStatus(`Valor base alterado para R$${CONFIG.valorBaseAposta.toFixed(2)}.`, 'info'); });
            inputMinas.addEventListener('change', (e) => { CONFIG.numeroDeMinas = parseInt(e.target.value, 10); this.updateStatus(`Número de minas alterado para ${CONFIG.numeroDeMinas}.`, 'info'); });
            inputCliques.addEventListener('change', (e) => { CONFIG.clicksParaRetirar = parseInt(e.target.value, 10); this.updateStatus(`Cliques para retirar alterado para ${CONFIG.clicksParaRetirar}.`, 'info'); });
        };
        ...
        // [CÓDIGO DO BOT DE MINES - FIM]
    }

    // --------------------------------------------------------------------------------------
    //  LÓGICA DO BOT PARA O JOGO DOUBLE
    // --------------------------------------------------------------------------------------
    function initDoubleBot() {
        // [CÓDIGO DO BOT DE DOUBLE - INÍCIO]
        const CONFIG = {
        ...
        const STATE = {
            ...
            estrategiasAtivas: { "2-em-1": true, "tendencia": false, "alternancia": true, },
        ...
        const Util = {
            ...
            acharProximo: tx => { const x = parseFloat(tx); return Object.keys(STATE.baseMap).reduce((acc, k) => { const d = Math.abs(parseFloat(k) - x); return d < acc.diff ? { key: k, diff: d } : acc; }, { key: null, diff: Infinity }); },
        ...
        const Bot = {
            analisar() { if (STATE.historico.length < 4) { UI.updateStatus("Aguardando mais resultados para analisar...", 'info'); return; } const ultimos = STATE.historico.slice(-4).map(e => e.cor); if (STATE.estrategiasAtivas["2-em-1"] && ultimos[2] === ultimos[3] && ultimos[2] !== ultimos[1]) { const corSinal = ultimos[2] === 'vermelho' ? 'preto' : 'vermelho'; this.gerarSinal(corSinal, "Dois-em-um"); return; } if (STATE.estrategiasAtivas["alternancia"] && ultimos[1] !== ultimos[2] && ultimos[2] !== ultimos[3] && ultimos[1] === ultimos[3]) { const corSinal = ultimos[3] === 'vermelho' ? 'preto' : 'vermelho'; this.gerarSinal(corSinal, "Alternância"); return; } if (STATE.estrategiasAtivas["tendencia"]) { const ultimos10 = STATE.historico.slice(-10).map(e => e.cor); const vermelhos = ultimos10.filter(c => c === 'vermelho').length; const pretos = ultimos10.filter(c => c === 'preto').length; if (vermelhos > pretos + 3) { this.gerarSinal('vermelho', "Tendência"); return; } if (pretos > vermelhos + 3) { this.gerarSinal('preto', "Tendência"); return; } } UI.updateStatus("🔍 Analisando padrões...", 'info'); },
        ...
        UI.showWinEffect = function() { const effect = document.createElement('div'); effect.style.cssText = `position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: 99998; pointer-events: none; animation: winEffect 1s ease-out; background: linear-gradient(to top, rgba(46, 204, 113, 0.3), transparent);`; document.body.appendChild(effect); setTimeout(() => effect.remove(), 1000); };
