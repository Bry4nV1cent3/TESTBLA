// ==UserScript==
// @name         Blaze Bot Avançado v5.1 - AutoMode + Painel Térmico
// @namespace    blaze-bot-v5
// @version      5.1
// @description  Bot com painel térmico, IA, Martingale, estratégias automáticas e coleta por translateX
// @author       Harpy
// @match        *://blaze.com/*
// @grant        none
// @license      MIT
...
(function () {
                ...
                if (m.attributeName === "style") {
                    ...
                    if (ref) processarResultado(ref.numero, ref.cor);
                        ...
                        if (diff <= CONFIG.limiteAproximacao) {
                            ...
                            STATE.baseTranslateXMap[tx] = ref;
        ...
        if (!ult || ult.numero !== numero) {
            ...
            if (STATE.aguardandoResultado) {
                ...
                if (!STATE.martingaleAtivo) {
                    ...
                    setTimeout(BOT.analisarEstrategias, 500);
                }
            }
        }
    }
    ...
    const UI = {
        criarPainel() {
            ...
            `;
            p.innerHTML = `
                <h3 style="text-align:center;">🔥 Blaze Bot v5.1</h3>
    ...
    setTimeout(() => {
        UI.criarPainel();
        monitorarRoleta();
        UI.atualizarStatus("🟢 Bot iniciado automaticamente", "sucesso");
        setTimeout(() => BOT.analisarEstrategias(), 1500);
    }, 2500);
})();
